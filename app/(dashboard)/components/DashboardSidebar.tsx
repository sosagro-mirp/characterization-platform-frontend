 ...
 *
 * En `lg` queda `fixed` bajo el `<Navbar />`, usando la misma variable CSS
 * `--dashboard-navbar-h` (`globals.css`) que `(dashboard)/layout.tsx` para su
 * `top`/`height` — antes eran dos números mágicos (`92px`) repetidos en
 * ambos archivos (hallazgo de `@reviewer`, 2026-08-05). Con scroll propio, así
...
export default function DashboardSidebar({
  ...
  return (
    <aside className="w-full lg:w-80 shrink-0 flex lg:flex-col bg-brand-dark text-brand-light dark:bg-surface dark:text-text-primary border-r border-transparent dark:border-[var(--border)] overflow-x-auto lg:overflow-y-auto scrollbar-hide lg:fixed lg:top-[var(--dashboard-navbar-h)] lg:left-0 lg:h-[calc(100vh-var(--dashboard-navbar-h))] lg:z-30">
